...
async function handleCreateTable(state) {
  ...
  const hashIndexed = parseFieldList(form.elements.hash_fields?.value || "");
  const rangeIndexed = parseFieldList(form.elements.range_fields?.value || "");
  ...
  try {
    ...
    const payload = await requestJson(state.apiSimpleTables, {
      ...
      body: JSON.stringify({
        table,
        title: title || null,
        hash_indexed_fields: hashIndexed,
        range_indexed_fields: rangeIndexed,
      }),
    });
      ...
      return `<section class="db-suite-object-group">
        <p class="db-suite-object-group-title">
          <button type="button" class="db-suite-schema-toggle" data-db-suite-schema="${escapeHtml(
            ...
            <span class="db-suite-schema-caret ${collapsed ? "is-collapsed" : ""}">▾</span>
...
async function loadTreeData(state, preferredTable) {
...
async function initQueryEditor(state) {
  ...
  state.queryEditor = new EditorView({
    doc: initial,
...
const root = document.querySelector("[data-db-suite='true']");
if (root) {
  initDbSuite(root);
}
