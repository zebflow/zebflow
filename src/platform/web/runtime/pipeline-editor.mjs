import {
  createGraphUI,
  createPipelineScene,
  DEFAULT_NODE_KIND_COLORS,
} from "/assets/libraries/zeb/graphui/0.1/runtime/graphui.bundle.mjs";
import {
  EditorView,
  basicSetup,
  javascript,
...
function setDraftState(state) {
  ...
  draft.textContent = isActive ? "active" : hasDraft ? "draft changed" : "inactive";
}
...
function setHeaderInfo(state) {
  ...
  if (trigger) {
    trigger.textContent = `trigger: ${String(state.currentMeta.trigger_kind || "-").toUpperCase()}`;
  }
  if (lockState) {
...
function attachNodeEditButtons(state) {
  ...
  root.querySelectorAll(".zgu-node").forEach((nodeEl) => {
    ...
    btn.addEventListener("click", (event) => {
      ...
      openNodeDialog(state, node);
    });
    nodeEl.appendChild(btn);

    const kind = String(node?.zfKind || "");
    const slug = String(node?.zfPipelineNodeId || "");
...
async function openWebRenderNodeDialog(state, node, refs) {
  ...
  const templateOptions = templates.map(normalizeTemplateOption).filter(Boolean);
  ...
  fieldsContainer.innerHTML = `
    <div class="pipeline-render-top">
      ...
      <label class="pipeline-editor-field">
        <span>Template</span>
        <input type="text" data-node-field="template_path_select" value="${selectedTemplate}" list="pipeline-render-template-list" placeholder="pages/blog/list.tsx" />
        <small class="pipeline-editor-field-help">Primary input for this node.</small>
      </label>
      <datalist id="pipeline-render-template-list">
        ${templateOptions
          .map((item) => `<option value="${String(item.value)}" label="${String(item.label)}"></option>`)
          .join("")}
      </datalist>
    </div>
    <div class="pipeline-render-workspace">
      <aside class="pipeline-render-sidebar" data-render-template-sidebar="true"></aside>
  ...
  const templateInput = fieldsContainer.querySelector('[data-node-field="template_path_select"]');
  ...
  function renderSidebar() {
    ...
    sidebar.innerHTML = templates
      .map((item) => {
        const relPath = String(item?.rel_path || "");
        const selected = relPath === String(templateInput?.value || "").trim();
    ...
    sidebar.querySelectorAll("[data-render-template-item]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        if (templateInput) {
          templateInput.value = relPath;
        }
        if (renderState.isDirty) {
          await saveTemplate().catch(() => {});
        }
        await loadTemplate(relPath).catch((err) => {
  ...
  if (templateInput) {
    templateInput.addEventListener("change", async () => {
      const relPath = String(templateInput.value || "").trim();
      if (renderState.isDirty) {
        await saveTemplate().catch(() => {});
      }
      await loadTemplate(relPath).catch((err) => {
  ...
  renderSidebar();
  if (selectedTemplate) {
    await loadTemplate(selectedTemplate).catch((err) => {
      setTemplateStatus(err?.message || String(err), "error");
    });
  } else {
  ...
  const onSubmit = async (event) => {
    ...
    const selected = String(templateInput?.value || "").trim();
    if (selected) {
      ...
      if (renderState.currentPath !== selected) {
        await loadTemplate(selected).catch(() => {});
      }
    }
    if (renderState.isDirty) {
      await saveTemplate().catch(() => {});
...
async function loadPipeline(state, id) {
  ...
  const scene = createPipelineScene(graph, {
    label: payload?.meta?.name || graph.id || "pipeline",
    kindColors: DEFAULT_NODE_KIND_COLORS,
  });

  state.graphApp = createGraphUI(graphRoot, {
    showHeader: false,
    showToolbox: false,
