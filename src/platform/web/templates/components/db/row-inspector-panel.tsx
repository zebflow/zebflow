...
function InspectedValue({ value, hasGeo, compact }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-ui-text-soft">{value.meta}</p>
      {map}
      <pre
        className="max-h-32 overflow-y-auto overflow-x-hidden rounded-md border border-ui-border/70 bg-ui-bg-muted/20 p-2 text-xs text-ui-text"
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {relations.busy ? <span className="text-[11px] text-ui-text-soft">Loading…</span> : null}
      </div>
      {relations.error ? (
...
function TableFacts({ activeTable, fieldCount, indexCount }) {
  return (
    <div className="grid grid-cols-2 gap-2 text-xs uppercase tracking-[0.12em] text-ui-text-soft">
      ...
      <span>Slug</span>
      <span className="truncate text-right normal-case tracking-normal text-ui-text">
        {activeTable.table}
      </span>
    </div>
  );
      ...
      {names.length ? (
        <div className="flex flex-wrap gap-2">
          {names.map((name) => (
            <span
              key={name}
              className="inline-flex rounded-full border border-ui-border/80 px-2 py-1 text-xs text-ui-text-soft"
            >
              {name}
            </span>
          ))}
...
export default function RowInspectorPanel({ node, value, relations, facts, hasGeo, children }) {
  ...
  return (
    <aside className="db-suite-value-panel">
      ...
      {record ? (
        <div
          className="flex min-h-0 flex-col gap-4 overflow-y-auto overflow-x-hidden px-3 py-3 text-sm text-ui-text"
          style={{ wordBreak: "break-word" }}
        >
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-ui-text">{label || slug}</p>
              <p className="truncate text-xs text-ui-text-soft">{slug}</p>
            </div>
            {children}
          </div>
