...
function queryHistoryKey(connectionSlug) {
  return `zf-qhist-${connectionSlug}`;
}

function loadQueryHistory(connectionSlug) {
  try {
    const raw = localStorage.getItem(queryHistoryKey(connectionSlug));
...
function saveQueryHistory(connectionSlug, sql) {
  if (!connectionSlug || !sql || !sql.trim()) {
    return;
  }
  const trimmed = sql.trim();
  try {
    const history = loadQueryHistory(connectionSlug).filter((q) => q !== trimmed);
    history.unshift(trimmed);
    if (history.length > HISTORY_MAX) {
      history.length = HISTORY_MAX;
    }
    localStorage.setItem(queryHistoryKey(connectionSlug), JSON.stringify(history));
  } catch (_) {
    // ignore storage errors
  }
}

function renderQueryHistory(state) {
  if (!state.historyEl) {
    return;
  }
  const history = loadQueryHistory(state.connectionSlug);
  if (!history.length) {
    state.historyEl.innerHTML = "";
    state.historyEl.style.display = "none";
    return;
  }
  state.historyEl.style.display = "";
  state.historyEl.innerHTML = history
    .map(
      (sql, index) =>
        `<button type="button" class="db-suite-history-item" title="${escapeHtml(sql)}" data-history-index="${index}">${escapeHtml(
          sql.length > 48 ? sql.slice(0, 46) + "\u2026" : sql
        )}</button>`
    )
    .join("");

  state.historyEl.querySelectorAll("[data-history-index]").forEach((btn) => {
    btn.addEventListener("click", () => {
      const index = Number(btn.getAttribute("data-history-index") || "0");
      const history = loadQueryHistory(state.connectionSlug);
      const sql = history[index];
      if (!sql || !state.queryEditor) {
        return;
      }
      try {
        const view = state.queryEditor;
        const { Text, EditorSelection } = (view.constructor as any);
        const newDoc = Text ? Text.of(sql.split("\n")) : sql;
        view.dispatch({
          changes: { from: 0, to: view.state.doc.length, insert: sql },
          selection: EditorSelection
            ? EditorSelection.cursor(sql.length)
            : { anchor: sql.length },
        });
      } catch (_) {
        // Fallback for non-CodeMirror editors
        if (state.queryEditorHost?.tagName === "TEXTAREA") {
          (state.queryEditorHost as HTMLTextAreaElement).value = sql;
        }
      }
    });
  });
...
async function runQuery(state) {
  ...
  try {
    ...
    saveQueryHistory(state.connectionSlug, sql);
    renderQueryHistory(state);
  } catch (err) {
...
async function initDbSuite(root) {
  ...
  const toolbar = root.querySelector(".db-suite-query-toolbar");
  let historyEl: HTMLElement | null = null;
  if (toolbar) {
    historyEl = document.createElement("div");
    historyEl.className = "db-suite-history-list";
    historyEl.style.display = "none";
    // Insert between the Run button and the status text
    const statusEl = toolbar.querySelector("[data-db-suite-query-status]");
    if (statusEl) {
      toolbar.insertBefore(historyEl, statusEl);
    } else {
      toolbar.appendChild(historyEl);
    }
  }
...
function scanDbSuiteRoots() {
  document.querySelectorAll("[data-db-suite='true']").forEach((root) => {
    ...
    initDbSuite(root).catch((err) => {
      console.error("db suite runtime init failed", err);
    });
  });
}

export function initProjectDbSuiteBehavior() {
