...
let javascriptExt = null;
let cssExt = null;
let autocompletionExt = null;
let snippetCompletionExt = null;
let linterExt = null;
let lintGutterExt = null;
let setDiagnosticsExt = null;
let oneDarkExt = null;
let createSplitPaneFn = null;
let debounceFn = null;
let runtimePromise = null;

async function ensureTemplateEditorRuntime() {
  if (
    editorViewCtor &&
    basicSetupExt &&
    javascriptExt &&
  ...
  runtimePromise = (async () => {
    if (typeof window === "undefined") {
      throw new Error("template editor runtime requires browser window");
    }
    ...
    const stateUtilRuntime = await import(stateUtilUrl.href);
    editorViewCtor = codeMirrorRuntime.EditorView;
    basicSetupExt = codeMirrorRuntime.basicSetup;
    javascriptExt = codeMirrorRuntime.javascript;
    cssExt = codeMirrorRuntime.css;
    autocompletionExt = codeMirrorRuntime.autocompletion;
    snippetCompletionExt = codeMirrorRuntime.snippetCompletion;
    linterExt = codeMirrorRuntime.linter;
    lintGutterExt = codeMirrorRuntime.lintGutter;
    setDiagnosticsExt = codeMirrorRuntime.setDiagnostics;
    oneDarkExt = codeMirrorRuntime.oneDark;
    createSplitPaneFn = interactRuntime.createSplitPane;
    debounceFn = stateUtilRuntime.debounce;
  })();
  return runtimePromise;
}

const LIBRARY_KEYWORDS_URL = "/assets/libraries/zeb/codemirror/0.1/keywords.json";
const ZEBFLOW_KEYWORDS_BASE = [
  { label: "Page", type: "type" },
  { label: "usePageState", type: "function" },
];

function buildDefaultKeywordSymbols() {
  const symbols = [...ZEBFLOW_KEYWORDS_BASE];
  if (!snippetCompletionExt) {
    return symbols;
  }
  symbols.push(
    snippetCompletionExt(
...
function iconForItem(item) {
  ...
  if (item.kind === "folder") {
    return '<svg viewBox="0 0 24 24" fill="none" class="template-tech-icon is-folder" aria-hidden="true"><path d="M4 7h6l2 2h8v8H4z" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"></path></svg>';
  }
  if (isTsx || item.file_kind === "page" || item.file_kind === "component") {
    return '<svg viewBox="0 0 24 24" fill="none" class="template-tech-icon is-react" aria-hidden="true"><circle cx="12" cy="12" r="1.6" fill="currentColor"></circle><ellipse cx="12" cy="12" rx="8.4" ry="3.4" stroke="currentColor" stroke-width="1.6"></ellipse><ellipse cx="12" cy="12" rx="8.4" ry="3.4" stroke="currentColor" stroke-width="1.6" transform="rotate(60 12 12)"></ellipse><ellipse cx="12" cy="12" rx="8.4" ry="3.4" stroke="currentColor" stroke-width="1.6" transform="rotate(-60 12 12)"></ellipse></svg>';
  }
  if (isTs || item.file_kind === "script") {
    return '<span class="template-tech-icon is-ts" aria-hidden="true">TS</span>';
  }
  if (isCss || item.file_kind === "style") {
    return '<i class="zf-devicon devicon-css3-plain colored" aria-hidden="true"></i>';
  }
  return '<i class="zf-devicon zf-icon-file" aria-hidden="true"></i>';
}
...
function editorExtensions(state, fileKind) {
  const extensions = [
    basicSetupExt,
    oneDarkExt,
    autocompletionExt({ override: [templateCompletions(state)] }),
    linterExt(() => []),
    lintGutterExt(),
  ...
  if (fileKind === "style") {
    extensions.push(cssExt());
  } else {
    extensions.push(javascriptExt({ jsx: true, typescript: true }));
  }
...
function applyDiagnostics(state, diagnostics) {
  ...
  try {
    state.view.dispatch(setDiagnosticsExt(state.view.state, cmDiagnostics));
  } catch (error) {
...
async function loadKeywordSymbols() {
  ...
  return [...buildDefaultKeywordSymbols(), ...symbols];
}
...
function createWorkspaceState(root) {
  return {
    ...
    currentFile: null,
    currentDiagnostics: [],
    keywordSymbols: buildDefaultKeywordSymbols(),
    gitItems: [],
    gitMap: new Map(),
    isDirty: false,
...
async function bootWorkspace(root) {
  await ensureTemplateEditorRuntime();
  ...
  state.keywordSymbols = await loadKeywordSymbols().catch((error) => {
    ...
    return buildDefaultKeywordSymbols();
  });
  state.markDraftReady = debounceFn(() => {
    if (!state.isDirty) {
      return;
    }
...
function scanTemplateWorkspaceRoots() {
...
export function initTemplateEditorBehavior() {
