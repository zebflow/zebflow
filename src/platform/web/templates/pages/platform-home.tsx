    ...
    <nav className="fixed top-0 w-full z-50 bg-white/95 backdrop-blur-sm shadow-sm py-3 border-b border-slate-200">
      <div className="max-w-6xl mx-auto px-6 flex justify-between items-center">
        ...
        <form method="post" action="/logout">
          <button className="px-4 py-2 bg-[#005B9A] text-white text-xs font-mono uppercase tracking-widest hover:bg-[#004A7A] transition-colors rounded-md">
            Logout
          </button>
        </form>
      </div>
        ...
        <section className="mb-8 bg-white border border-slate-200 rounded-xl p-5">
          <form method="post" action="/home/projects/create" className="grid md:grid-cols-3 gap-3">
            <input
              ...
              required
              className="rounded-md border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:border-[#005B9A]"
            />
            <input
              name="title"
            ...
            <button
              type="submit"
              className="px-5 py-2 bg-[#005B9A] text-white text-xs font-bold uppercase tracking-widest hover:bg-[#004A7A] transition-colors rounded-md"
            >
              Create Project
            </button>
          </form>
