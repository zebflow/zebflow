                      ...
                      <div className="db-suite-grid-wrap">
                        <StudioTable variant="dbGrid" data-db-suite-table-preview-table="true">
                          ...
                          <tbody data-db-suite-table-preview-body="true">
                            {previewRows.map((row, rowIndex) => (
                              <tr key={`row-${rowIndex}`}>
                                {(Array.isArray(row) ? row : []).map((cell, cellIndex) => {
                                  ...
                                  return (
                                    <StudioTd key={`cell-${rowIndex}-${cellIndex}`} onClick={() => onCellInspect(colName, rowIndex, cell)}>
                                      {stringifyCell(cell)}
                                    </StudioTd>
                                  );
                                })}
                            ...
                            {!previewRows.length ? (
                              <tr>
                                <StudioTd colSpan={Math.max(previewColumns.length, 1)}>
                        ...
                        <StudioTable variant="dbGrid">
                          <StudioThead>
                            <tr data-db-suite-query-head="true">
                              {queryColumns.map((col, index) => <StudioTh key={`qcol-${col}-${index}`}>{col}</StudioTh>)}
                            </tr>
                          </StudioThead>
