...
export function initInstallCatalogBehavior() {
  ...
  requestAnimationFrame(() => {
    ...
    async function loadCatalog() {
      ...
      componentList.innerHTML = '<div style="padding:16px;color:var(--muted-foreground);font-size:12px;">Loading…</div>';
      try {
        ...
        componentList.innerHTML = '<div style="padding:16px;color:var(--muted-foreground);font-size:12px;">Failed to load catalog.</div>';
      }
    }
    ...
    function renderList() {
      ...
      if (catalogData.length === 0) {
        componentList.innerHTML = '<div style="padding:16px;color:var(--muted-foreground);font-size:12px;">No components found.</div>';
        return;
      }
      componentList.innerHTML = catalogData.map(comp => `
        <label style="display:flex;align-items:center;gap:6px;padding:5px 6px;border-radius:5px;cursor:pointer;font-size:12px;background:${comp.installed ? 'var(--muted)' : ''}" title="${comp.description}">
          <input type="checkbox" data-component-name="${comp.name}" ${comp.installed ? "" : ""} style="accent-color:var(--info);" />
          <span style="flex:1;color:var(--foreground)">${comp.name}</span>
          ${comp.installed ? '<span style="font-size:10px;color:#22c55e;">✓</span>' : ''}
          <span style="font-size:10px;color:var(--muted-foreground);text-transform:capitalize;">${comp.category}</span>
        </label>
      `).join("");
    }
