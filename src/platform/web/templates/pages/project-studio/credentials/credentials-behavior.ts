...
function createHelpTooltip(text: string): HTMLElement {
  ...
  icon.className = "inline-flex text-muted-foreground transition-colors duration-150 group-hover:text-foreground group-focus-within:text-foreground";
  ...
  content.className = "pointer-events-none invisible absolute bottom-[calc(100%+8px)] left-1/2 z-50 min-w-40 max-w-60 -translate-x-1/2 rounded-md border border-border bg-gray-800 px-2.5 py-1.5 text-[11px] font-normal leading-[1.5] tracking-normal text-gray-100 opacity-0 shadow-lg transition-[opacity,visibility] duration-150 group-hover:visible group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100";
...
function addTagChip(container: HTMLElement, value: string) {
  ...
  chip.className = "inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs bg-muted border border-border text-foreground";
  chip.setAttribute("data-tag-value", v);
  const label = document.createElement("span");
  label.textContent = v;
  const removeBtn = document.createElement("button");
  removeBtn.type = "button";
  removeBtn.className = "text-muted-foreground hover:text-danger leading-none cursor-pointer bg-transparent border-0 p-0";
