...
export function dependencyStatusClass(status) {
  if (status === "resolved") return "border-info text-info";
    ...
    <section className="border border-border rounded-lg bg-card overflow-hidden">
      <header className="flex items-start justify-between gap-3 border-b border-border px-4 py-3">
        <div className="min-w-0">
          <h3 className="text-[0.83rem] font-semibold tracking-[0.01em] text-foreground">Dependency Lock</h3>
        ...
        <span className="inline-flex items-center border border-border bg-muted px-2 py-[0.38rem] text-[0.66rem] font-mono uppercase tracking-[0.12em] text-muted-foreground">
          {report?.ok ? "Resolved" : `${report?.problems ?? 0} problem(s)`}
        </span>
      </header>
      <div className="px-4 py-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
        <div className="flex items-center gap-4 text-[0.74rem] text-muted-foreground">
          <span><strong className="text-foreground">{report?.resolved ?? 0}</strong> resolved</span>
          <span><strong className={report?.problems ? "text-warning" : "text-foreground"}>{report?.problems ?? 0}</strong> problems</span>
      ...
      {error ? <p className="border-b border-border py-3 text-[0.74rem] text-warning">{error}</p> : null}
      {items.length ? (
          ...
          <table className="w-full border-collapse text-left text-[0.74rem]">
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={`${item?.family}-${item?.name}-${index}`} className="border-b border-border last:border-b-0">
                  <td className="min-w-[15rem] px-2 py-2.5 align-top">
                    <code className="text-foreground">{item?.name}</code>
                    <p className="mt-1 text-[0.69rem] leading-[1.4] text-muted-foreground">{item?.message}</p>
                    {Array.isArray(item?.definitions) && item.definitions.length ? (
                      <p className="mt-1 font-mono text-[0.66rem] text-muted-foreground">{item.definitions.join(", ")}</p>
                    ) : null}
                  </td>
                  <td className="whitespace-nowrap px-2 py-2.5 align-top text-muted-foreground">{String(item?.family ?? "").replaceAll("_", " ")}</td>
                  <td className="whitespace-nowrap px-2 py-2.5 align-top font-mono text-foreground">{item?.version}</td>
                  <td className="whitespace-nowrap px-2 py-2.5 align-top text-muted-foreground">{item?.source}</td>
        ...
        <p className="py-5 text-[0.76rem] text-muted-foreground">This project has no external runtime dependencies.</p>
      )}
      </div>
    </section>
  );
