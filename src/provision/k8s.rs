...
fn render_office_container_block(cfg: &ClusterConfig, office_id: &str) -> String {
    let mut lines = vec![
        ...
        "            - name: ZEBFLOW_HEALTH_HOST".to_string(),
        "              value: \"0.0.0.0\"".to_string(),
        "            - name: ZEBFLOW_HEALTH_PORT".to_string(),
...
fn default_image() -> String {
    format!("zebflow/zebflow:{APP_VERSION}")
}
...
mod tests {
    ...
    fn office_manifests_use_health_and_ready_probes() {
        ...
        assert!(office.contains("path: /health/runtime"));
        assert!(office.contains("port: health"));
        assert!(office.contains("containerPort: 10611"));
        assert!(office.contains("name: ZEBFLOW_HEALTH_PORT"));
    ...
    fn resource_suffix_renders_parallel_workloads_without_managed_secret() {
        ...
        assert!(management.contains(
            "MANAGED_WORKLOADS: \"statefulsets/branch-a-v6,statefulsets/office-main-v6\""
        ));

        let office = fs::read_to_string(root.join("branch-a.yaml")).expect("office");
        assert!(office.contains("name: branch-a-v6"));
        assert!(office.contains("name: data-branch-a-v6-0"));
        ...
        assert!(office.contains("value: \"branch-a\""));

        let auto_update = fs::read_to_string(root.join(AUTO_UPDATE_FILE)).expect("auto-update");
        assert!(auto_update.contains("name: zebflow-auto-updater-v6"));
        assert!(auto_update.contains("- branch-a-v6"));
        assert!(
            auto_update.contains("value: \"statefulsets/branch-a-v6,statefulsets/office-main-v6\"")
        );
    }
