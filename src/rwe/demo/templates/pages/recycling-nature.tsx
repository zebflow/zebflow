      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <article className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl">
          ...
          <ul className="mt-4 space-y-3">
            {(input.recycleTips || []).map((tip) => (
              <li key={tip.id} className="rounded-lg border border-zinc-800 bg-zinc-950 px-4 py-3">
        ...
        <article hydrate="interaction" className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-2xl space-y-3">
          ...
          <p className="text-sm">
            Live pledges:
            <span className="font-semibold text-emerald-300">{input.ui?.pledges ?? 0}</span>
          </p>
