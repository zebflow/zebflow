...
mod tests {
    ...
    fn normalizes_site_root_and_page_paths() {
        assert_eq!(
            normalize_site_root_rel_path("private", "static/musicsite").unwrap(),
            "private/static/musicsite"
        );
        assert_eq!(
            page_rel_path_from_site_root("private/static/musicsite", "a/aurora/index.html").unwrap(),
            "private/static/musicsite/a/aurora/index.html"
        );
        assert!(normalize_page_output_path("../escape.html").is_err());
        assert_eq!(
            site_manifest_rel_path("private/static/musicsite"),
    ...
    fn normalizes_deploy_paths_and_routes() {
        ...
        assert_eq!(
            ...
            Some("https://db.docs.example/docs/basic/query/".to_string())
        );
    }
