...
pub fn definition() -> NodeDefinition {
    NodeDefinition {
        ...
        dsl_flags: vec![
            ...
            DslFlag { flag: "--body".to_string(), config_key: "body".to_string(), description: "Request body — a literal or {{ expr }}. A whole {{ }} carries its typed value, so an object stays an object. With a secure_request profile it is sent when the profile's Body Template is blank; a profile that declares a body owns it.".to_string(), kind: DslFlagKind::Scalar, required: false },
...
impl NodeHandler for Node {
    ...
    async fn execute_async(
        ...
        if !prepared.headers.keys().any(|k| k.eq_ignore_ascii_case("x-request-id")) {
...
mod tests {
    ...
    fn secure_request_with_a_blank_body_template_sends_the_nodes_body() {
        ...
        assert_eq!(
            ...
            Some("Bearer rw-test-key-4f1e9c2b7a60")
        );
