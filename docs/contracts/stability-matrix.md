...
| 2 | P0 | [Project directory](./project-directory.md) | Project repository, data, files, runtime, and generated paths | Policy only | `src/platform/adapters/file/mod.rs`, project services | Review | Applied and live-verified: `data/` splits into store, cache, recovery, and logs; `ProjectFileLayout` carries all four; every named writer moved; a pre-tier project migrates transparently on first touch, atomically, idempotently. `data/nodes/` was investigated for the `cache` tier and live-tested to disprove it: the only wired production install path (`POST /nodes/install`, an inline artifact body) writes bytes `zeb.lock` cannot reproduce, verified by installing a real bundle, deleting `data/nodes/`, and confirming the platform's own repair path cannot bring it back. It stays unclassified — this row cannot move toward candidate on `data/nodes/` alone; either the install path starts retaining what it needs to reconstruct, or the directory is classified as something other than cache |
...
| 14 | P1 | [Hub package](./kinds/hub-package/README.md) | One published release: what installing it requires. Presentation, publisher identity, and provenance live in mutable storage beside it | `zebflow.com/v1` `HubPackage`; release version is independent | `src/contracts/kinds/hub_package.rs`, `src/platform/services/hub.rs`, platform models | Review | Releases are immutable, the spec is typed and bounded, referenced artifacts resolve and fetch per channel, a publish over 1 MiB references rather than carries, and publish and both install gates read one document through one reader; `spec.layout` needs exercise beyond the week it was written before a freeze |
