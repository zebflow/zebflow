...
| 13b | P1 | [RWE source](./kinds/rwe-source/README.md) | One authored page, component, behavior script, or stylesheet | none; source file, no envelope | `src/rwe/core/compiler.rs` | Review | Spec sealed 2026-08-30: one default export per page, four import forms only, imports confined to the template root, add refuses a collision. Code catch-up owed: relative imports are resolved today and must be refused instead, with the message naming the `@/` replacement, because the bundler does not carry what they resolve to and the files are mostly written by language models that count `../` wrongly. Still open: behavior-script and stylesheet import rules |
...
| 16 | P2 | [Invocation and temporary state](./kinds/invocation-record/README.md) | Invocation records, traces, node payloads, caches, and temporary files | No common version; database record, no envelope | Pipeline runtime and invocation services | Review | Spec sealed 2026-08-29: bounded by newest-N (default 20) and optional maximum age, per pipeline overriding per project; secrets are redacted by the node's own declaration first, with name matching kept as a fallback. Code catch-up owed: `NodeDefinition` gains the optional secret flag (amended 2026-08-29) and the engine reads it — today `is_sensitive_trace_config_key` matches names only, so a `db_url` holding `postgres://user:pass@host/db` is written to the run history in full. Still open: no maximum trace byte size though `trace_bytes` is already measured, deletion undefined, no rule on which roles may read a trace, high-throughput behaviour undefined |
