...
| 17 | P1 | Library definition | Installed library manifests, versions, URLs, integrity, and project enablement | `zebflow.com/v1` `RweLibraryManifest` | `src/platform/services/library.rs`, library catalog files | Partial | Strict manifests are enforced at startup; require complete integrity values before external libraries can be enabled |
